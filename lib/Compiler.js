...
Compiler.prototype.build = function (instructions, cb) {
  
  // Validate usage
  Compiler.usage.build(arguments);

  // Get source markdown files
  // TODO

  // Transform markdown (split files, etc.)
  // TODO

  // Compile markdown files into HTML
  // Iterate over all specified file groups.
  // async.forEachLimit(this.files, 25, function (file, next) {
...
Compiler.usage = {
  ...
  build: function (args) {
    ...
    var instructions = args[0];
    
    if (!_.isArray(instructions)) {
      throw new Error('Invalid usage: `instructions` argument should be an array of objects.  Instead got:'+util.inspect(instructions));
    }
    var usageErrors = _.reduce(instructions, function (memo, instruction, i) {
...
// Compiler.prototype.build = function(sgd49adsG?A, finalCB) {
