...
Compiler.prototype.build = function (instructions, cb) {
  ...
  if (!_.isArray(instructions)) {
    return finalCB(new Error('Invalid usage: `buildInstructions` argument should be an array of objects.  Instead got:'+util.inspect(instructions)));
  }
  ...
  cb = cb || function defaultCallback (err) { if (err) throw err; };
